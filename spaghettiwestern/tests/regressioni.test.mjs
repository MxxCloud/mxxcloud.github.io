...
test('spaghetti western non tocca i salvataggi di ultimo raccolto: le sue chiavi hanno un prefisso suo',()=>{
  ...
  assert.match(leggi('regole/salvataggio.js'),/const PREFISSO = "spaghettiwestern\/";/);
  assert.match(leggi('regole/sincronia.js'),/const CHIAVE = "spaghettiwestern\/sincronia";/);
  assert.match(leggi('regole/sincronia.js'),/const CHIAVE_VISTO = "spaghettiwestern\/sincronia-visto";/);
  assert.match(leggi('motore/suono.js'),/const CHIAVE = "spaghettiwestern\/volume";/);
  for(const f of ['regole/salvataggio.js','motore/suono.js','sw.js'])assert.doesNotMatch(leggi(f),/"ultimo-raccolto[\/-]/,f);
});
